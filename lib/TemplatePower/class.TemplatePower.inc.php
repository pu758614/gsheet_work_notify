...
// +----------------------------------------------------------------------+
//
// $Id: class.TemplatePower.inc.php,v 1.1.1.1 2013/09/14 07:04:59 athena Exp $
...
class TemplatePower extends TemplatePowerParser
{
  ...
  /**********
    ...
    {
        if(array_key_exists($blockname,$this->parent)){
            $parent = &$this->content[ $this->parent[$blockname] .'_'. $this->index[$this->parent[$blockname]] ];

            $lastitem = (is_array($parent) || is_object($parent)) ? sizeof( $parent ) : 0;
            $lastitem > 1 ? $lastitem-- : $lastitem = 0;
